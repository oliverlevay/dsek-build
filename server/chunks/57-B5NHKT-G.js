...
import { M as documents_governing_errors_notFound, N as documents_governing_documentUpdated } from './messages-D8OfyZiq.js';
...
const load = async ({ locals, params }) => {
  ...
  if (!governingDocument) {
    throw error(404, documents_governing_errors_notFound());
  }
  return {
    governingDocument,
...
const actions = {
  update: async (event) => {
    ...
    throw redirect(
      "/documents/governing",
      {
        message: documents_governing_documentUpdated(),
        type: "success"
      },
      event
    );
